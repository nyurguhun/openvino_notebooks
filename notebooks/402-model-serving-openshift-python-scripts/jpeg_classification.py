...
print('Images list file: {}\n'.format(args.get('images_list')))
...
for line in lines:
    ...
    if int(label) == ma:
        matched += 1
        mark_message = f"Correct classification: {classes.imagenet_classes[ma]}"
    else:
        mark_message = f"Incorrect: {classes.imagenet_classes[ma]}. Should be {classes.imagenet_classes[int(label)]}"
    i += 1
    print(f'{i}. {mark_message}')
    # for object classification models show imagenet class
    print('Processing time: {:.2f} ms; speed: {:.2f} fps\n'.format(round(duration, 2), round(1000 / duration, 2)))
